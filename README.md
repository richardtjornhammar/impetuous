...
"Connection" algorithms, such as the two mentioned, evaluate every distance and connect them if there is any distance overlap for a specific distance. "Link" algorithms try to determine the number of clusters for all unique distances by reducing and ignoring some connections to already linked constituents of the system in accord with a chosen heuristic. 
